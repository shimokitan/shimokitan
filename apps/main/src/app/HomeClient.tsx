        ...
        <div className="grid grid-cols-2 md:grid-cols-5 md:grid-rows-7 gap-3 h-auto md:h-full">
            ...
            <div className="col-span-2 md:col-span-3 md:row-span-3 relative group rounded-xl overflow-hidden border border-zinc-800/80 hover:border-violet-500/50 transition-all min-h-[45vh] md:min-h-0 bg-zinc-950 shadow-2xl">
                ...
                <div className="absolute inset-x-0 bottom-0 p-8 z-10">
                    ...
                    <h2 className="text-6xl md:text-8xl font-black tracking-tighter leading-[0.8] text-white mb-4 italic">
                        VOID<br />
                    ...
                    <p className="text-zinc-300 text-xs font-mono leading-relaxed max-w-xs mb-8 uppercase tracking-wide">
                        {heroArtifact?.description || "Archive the friction. Reject the sterile. Embrace the noise of the street."}
                    </p>
            ...
            <BentoCard className="col-span-2 md:col-span-1 md:row-span-3 overflow-visible min-h-[340px] md:min-h-0" title="Recent Shards" icon="lucide:star">
            ...
            <BentoCard className="col-span-1 md:col-span-1 md:row-span-3 px-3" title="District" icon="lucide:map-pin">
                <div className="flex flex-col h-full justify-between py-2">
                    {/* Header: Signal Status */}
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1.5">
                            <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.5)]" />
                            <span className="text-[8px] font-mono text-emerald-500 font-black tracking-widest uppercase">SIGNAL_STABLE</span>
                        </div>
            ...
            <BentoCard className="col-span-2 md:col-span-1 md:row-span-2 md:col-start-3 md:row-start-4" title="Recent Echoes" icon="lucide:ghost">
                    ...
                    {recentZines.map((zine) => (
                        <Link key={zine.id} href={`/artifacts/${zine.artifact_id}`} className="relative group rounded-lg overflow-hidden border border-zinc-800">
                            ...
                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                <span className="text-[8px] font-black text-white uppercase tracking-tighter">VIEW_SHARD</span>
                            </div>
                        </Link>
                    ))}
                </div>
            ...
            {featuredArtifact && (
                <Link href={`/artifacts/${featuredArtifact.id}`} className="col-span-1 md:col-span-1 md:row-span-2 md:col-start-1 md:row-start-4">
                    <BentoCard className="h-full" title="In The Pit" icon="lucide:flame">
            ...
            <Link href={`/artifacts/${spotlightArtifacts[1]?.id || heroArtifact?.id}`} className="col-span-2 md:col-span-1 md:row-span-2 md:col-start-2 md:row-start-4">
                <BentoCard className="h-full px-3" title="Playlist" icon="lucide:disc">
            ...
            <BentoCard className="col-span-2 md:col-span-3 md:row-span-2 md:col-start-1 md:row-start-6 overflow-hidden p-0" minimal>
                <div className="flex h-full w-full">
                    ...
                    <div className="flex-1 p-6 flex flex-col justify-center bg-zinc-950/50 backdrop-blur-sm">
                        <Badge variant="gold" className="mb-2">EDITORIAL_SIGIL</Badge>
                        <h3 className="text-2xl font-black italic tracking-tighter uppercase mb-2">Narrative Collapse.</h3>
                        <p className="text-zinc-500 text-xs italic leading-relaxed line-clamp-2">
                            The district is not a location; it is a frequency. When the signals overlap, the architecture of the real begins to dissolve into pure data resonance.
                        </p>
                    </div>
                </div>
            </BentoCard>
